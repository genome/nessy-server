...
urlpatterns = patterns('',
    ...
    url(r'^v1/locks/(?P<resource_name>\w{1,64})/$',
        views_v1.LockDetailView.as_view(), name='lock-detail'),
    url(r'^v1/locks/(?P<resource_name>\w{1,64})/owner/$',
        views_v1.OwnerDetailView.as_view(), name='lock-owner'),
    url(r'^v1/locks/(?P<resource_name>\w{1,64})/requests/$',
        views_v1.RequestListView.as_view(), name='lock-requests'),
    url(r'^v1/locks/(?P<resource_name>\w{1,64})/requests/(?P<pk>\w+)/$',
        views_v1.RequestDetailView.as_view(), name='request-detail'),
)
